      ...
      if (i >= expectedTokens.length) {
        ...
        if (actual.value != expected.value) {
          fail(s"Token mismatch at index $i:\nExpected: ${formatToken(expected)}\nActual:   ${formatToken(actual)}\n" +
            s"Context:\n${renderUnderline(input, lineMap, expected.begin.value, "expected " + expected.value)}\n" +
            s"${renderUnderline(input, lineMap, actual.begin.value, "actual " + actual.value)}")
        }
        ...
        if (actual.positionTuple != expected.positionTuple) {
          fail(s"Position mismatch for token '${actual.value}' at index $i:\nExpected: ${expected.begin}:${expected.end}\nActual:   ${actual.begin}:${actual.end}\n" +
            s"Context:\n${renderUnderline(input, lineMap, expected.begin.value, "expected start")}\n" +
            s"${renderUnderline(input, lineMap, actual.begin.value, "actual start")}")
        }
      }
    }
  }
